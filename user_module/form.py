...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(label='用户名或邮箱',required=True,widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入用户名或者邮箱'}))
...
class ChangeNicknameForm(forms.Form):
    nickname = forms.CharField(label='新昵称',required=True,
    ...
    def clean_nickname(self):
        ...
        if nickname =='':
            raise forms.ValidationError('新的昵称为空')
        return nickname

class BindEmailForm(forms.Form):
    email = forms.EmailField(label='email',required=True,
                                widget=forms.EmailInput(attrs={'class':'form-control','placeholder':'请输入email'}))
    verification_code = forms.CharField(label='验证码',required=False,
                                widget=forms.TextInput(attrs={'class':'form-control','placeholder':"点击'发送验证码'发送到邮箱"}))
    def __init__(self,*args, **kwargs):
        if 'request' in kwargs:
    ...
    def clean(self):
        ...
        if self.request.user.email != '':
            raise forms.ValidationError('用户已经绑定邮箱了')
...
class ChangePasswordForm(forms.Form):
    ...
    def clean(self):
        ...
        if new_password != new_password_again or new_password=='':
            raise forms.ValidationError('两次新密码可能不一致')
        return self.cleaned_data
...
class ForgetPasswordForm(forms.Form):
    ...
    verification_code = forms.CharField(label='验证码',required=True,
                                widget=forms.TextInput(attrs={'class':'form-control','placeholder':"点击'发送验证码'发送到邮箱"}))
    ...
    def clean_verification_code(self):
        ...
        if verification_code == "":
            raise forms.ValidationError('验证码不能为空')
        # 判断验证码是否正确
        code = self.request.session.get('forget_email_code','')
        if not (code !='' and code == verification_code):
            raise forms.ValidationError('验证码不正确')
        return verification_code
